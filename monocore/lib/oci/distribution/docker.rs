...
pub struct DockerRegistry {
    ...
    /// The path where image layers are downloaded.
...
impl DockerRegistry {
    ...
    /// ## Arguments
    ///
    /// * `image_download_path` - The path where downloaded image layers will be stored
    /// * `oci_db_path` - The path to the SQLite database that stores OCI-related metadata
    pub async fn new(
        image_download_path: impl Into<PathBuf>,
        ...
        Ok(Self {
            client,
            image_download_path: image_download_path.into(),
    ...
    fn get_downloaded_file_size(&self) -> u64 {
        // If the file does not exist, return 0 indicating no bytes have been downloaded
        if !self.image_download_path.exists() {
            return 0;
        }

        self.image_download_path.metadata().unwrap().len()
    }
    ...
    async fn download_image_blob(
        ...
        if let Some(parent) = self.image_download_path.parent() {
            fs::create_dir_all(parent).await?;
        }

        // Get the size of the already downloaded file if it exists
        let downloaded_size = self.get_downloaded_file_size();
        ...
        let mut file = if downloaded_size == 0 {
            ...
            tracing::info!(
                ...
                self.image_download_path.display()
            );
            return Ok(());
        };
        ...
        let actual_hash =
            hex::encode(utils::get_file_hash(&self.image_download_path, algorithm).await?);

        // Delete the already downloaded file if the hash does not match
        if actual_hash != expected_hash {
            fs::remove_file(&self.image_download_path).await?;
