...
/// Policy for how the supervisor should handle child process termination
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SupervisorPolicy {
    /// Exit when child process exits (one-shot execution)
    #[default]
    OneShot,

    /// Restart the child process when it exits with a failure status
    RestartOnFailure,
}
    ...
    M: ProcessMonitor + Send,
{
    ...
    pub async fn start(&mut self) -> MonoutilsResult<()> {
        loop {
            ...
            let _supervisor_log =
                RotatingLog::new(self.log_dir.join(SUPERVISOR_LOG_FILENAME)).await?;
            ...
            let should_restart = tokio::select! {
                status = child.wait() => {
                    ...
                    let should_restart = if let Ok(status) = status {
                        if status.success() {
                            tracing::info!(
                            ...
                            matches!(self.policy, SupervisorPolicy::RestartOnFailure)
                        }
                    } else {
                        // Consider unexpected errors as failures
                        matches!(self.policy, SupervisorPolicy::RestartOnFailure)
                    };
            ...
            tracing::info!("Restarting child process due to RestartOnFailure policy");
        }
    }
}
